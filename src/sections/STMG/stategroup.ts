...
export class StateGroup {
    ...
    constructor(stream: FileStream) {
        ...
        console.log(
            `state Group #${this.stateGroupID} with ${this.customTransistionTimeCount} custom times`
        );
        this.customTransistionTimes = [];
        for (let i = 0; i < this.customTransistionTimeCount; i++) {
            this.customTransistionTimes.push(
                new CustomTransistionTime(new FileStream(stream.readData(12)))
            );
        }
    }
}
